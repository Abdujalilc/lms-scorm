...
{
    public class prerequisitesType : Altova.Node
    {
        #region Forward constructors
        public prerequisitesType() : base() { SetCollectionParents(); }
        public prerequisitesType(XmlDocument doc) : base(doc) { SetCollectionParents(); }
        public prerequisitesType(XmlNode node) : base(node) { SetCollectionParents(); }
        public prerequisitesType(Altova.Node node) : base(node) { SetCollectionParents(); }
        #endregion // Forward constructors

        #region Value accessor methods
        public prerequisiteStringType GetValue()
        {
            return new prerequisiteStringType(GetDomNodeValue(domNode));
        }

        public void setValue(prerequisiteStringType newValue)
        {
            SetDomNodeValue(domNode, newValue.ToString());
        }

        public prerequisiteStringType Value
        {
            get
            {
                return new prerequisiteStringType(GetDomNodeValue(domNode));
        ...
        {
            prerequisitesType parent;
            public prerequisitesType Parent
            {
                set
                {
                    parent = value;
        ...
        public class type2Enumerator : IEnumerator
        {
            int nIndex;
            prerequisitesType parent;
            public type2Enumerator(prerequisitesType par)
            {
                parent = par;
                nIndex = -1;
            }
