...
function WriteToDebug(strInfo) {
    ...
    if (blnDebug) {
        ...
        if (winDebug && !winDebug.closed && my_div) {
            var inner_div = winDebug.document.createElement('div');
            inner_div.appendChild(winDebug.document.createTextNode(strLine));
            inner_div.appendChild(winDebug.document.createElement("br"));
            inner_div.appendChild(winDebug.document.createElement("br"));
            my_div.appendChild(inner_div);

            winDebug.document.body.appendChild(my_div);
        }
    }
    return;
}
...
function ShowDebugWindow() {
    var renderLog = function () {
        ...
        my_div.setAttribute("class", "container");
        for (i = 0; i < len; i += 1) {
            var inner_div = winDebug.document.createElement('div');
            inner_div.appendChild(winDebug.document.createTextNode(aryDebug[i]));
            inner_div.appendChild(winDebug.document.createElement("br"));
            inner_div.appendChild(winDebug.document.createElement("br"));
            my_div.appendChild(inner_div);
            winDebug.document.body.appendChild(my_div);
    ...
    winDebug = window.open("/blank.html", "Debug", "width=600,height=300,resizable");
    var doc = winDebug.document;
